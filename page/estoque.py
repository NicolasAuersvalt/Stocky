...
class EstoquePage(Page):
    ...
    # --- Funções de Banco de Dados e Exportação (sem alterações) ---
    ...
    def _registrar_transacao_e_atualizar_estoque(self, prod_id: int, usuario_id: int, tipo_acao: str, qtd_alterada: int, preco_unitario: float):
        ...
        try:
            ...
            sql_log = "INSERT INTO historico_transacoes (produto_id, usuario_id, tipo, quantidade_alterada, preco_transacao, quantidade_anterior, quantidade_nova) VALUES (%s, %s, %s, %s, %s, %s, %s)"
    ...
    def _buscar_historico_completo(self, empresa_id: int):
        ...
        if conn:
            try:
                cursor = conn.cursor(dictionary=True)
                sql_query = "SELECT h.data_transacao, p.nome AS produto, u.nome AS usuario, h.tipo, h.quantidade_alterada, h.preco_transacao, h.quantidade_anterior, h.quantidade_nova FROM historico_transacoes h JOIN produtos p ON h.produto_id = p.id JOIN usuarios u ON h.usuario_id = u.id WHERE p.empresa_id = %s ORDER BY h.data_transacao DESC"
    ...
    def show(self):
        ...
        # --- SEÇÃO DE GERENCIAMENTO DE PRODUTOS (AGORA NO TOPO) ---
        st.markdown("---")
        ...
        for produto in produtos:
            ...
            with col_info: st.write(f"**{produto['nome']}**"); st.write(f"ID: {produto['id']} | Preço: R$ {produto['preco']:.2f}")
            with col_qtd: st.write(f"Quantidade: {produto['quantidade']}")
            with col_compra:
            ...
            if st.session_state.acao_produto == produto["id"]:
                ...
                with col_cancel:
                    if st.button("Cancelar", key=f"cancel_{produto['id']}"): st.session_state.acao_produto = None; st.session_state.modo = None; st.rerun()
        ...
        if historico_data:
            ...
            with col1:
                st.download_button(label="Exportar para .CSV", data=df_historico.to_csv(index=False, sep=';').encode('utf-8'), file_name=f"historico_{datetime.now().strftime('%Y%m%d')}.csv", mime='text/csv')
            with col2:
                pdf_bytes = self._gerar_pdf(df_historico)
                st.download_button(label="Exportar para .PDF", data=pdf_bytes, file_name=f"historico_{datetime.now().strftime('%Y%m%d')}.pdf", mime='application/pdf')
        else:
