...
class AdmPage:
    def __init__(self):
        ...
        if self.conn:
            ...
            st.error("Falha na conexão com o banco de dados."); st.stop()
    ...
    def _conectar():
        try:
            ...
            st.error(f"Erro ao conectar ao banco de dados: {err}"); return None

    # --- Funções de Histórico e Geração de PDF ---

    def _buscar_historico_completo(self, empresa_id: int):
        """Busca um histórico detalhado, agora incluindo os novos campos de valor."""
        conn = self._conectar()
        if conn:
            try:
                cursor = conn.cursor(dictionary=True)
                # ALTERADO: Query igual à da página de estoque para incluir os novos campos
                sql_query = """
                    SELECT 
    ...
    def _mostrar_formulario_produto(self, produto=None):
        ...
        with st.form(key="form_produto"):
            ...
            quantidade = col1.number_input("Quantidade em Estoque", min_value=0, step=1, value=produto["quantidade"] if is_alteracao else 0)
            preco = col2.number_input("Preço Unitário Padrão (R$)", min_value=0.0, format="%.2f", value=float(produto["preco"]) if is_alteracao else 0.0)
    ...
    def show(self):
        ...
        if 'empresa_selecionada' not in st.session_state or st.session_state.empresa_selecionada is None:
            ...
            if not opcoes:
                st.warning("Nenhuma empresa encontrada para gerenciar."); return
            selecao = st.selectbox("Empresas Disponíveis", opcoes.keys())
            if st.button("Gerenciar Empresa"):
                st.session_state.empresa_selecionada = opcoes[selecao]; st.rerun()
            return
        ...
        # --- Seção de Gerenciamento de Produtos (Vem primeiro) ---
        st.markdown("---"); st.subheader("Gerenciar Produtos da Empresa")
        ...
        if st.session_state.acao == 'cadastrar':
            ...
            if st.button("✚ Cadastrar Novo Produto"):
                st.session_state.acao = 'cadastrar'; st.rerun()
        ...
        # --- Seção de Histórico e Exportação (AGORA NO FIM DA PÁGINA) ---
        st.markdown("---"); st.subheader("Histórico de Movimentações da Empresa")
        ...
        if historico_data:
            ...
            col1, col2 = st.columns(2)
            with col1:
                st.download_button("Exportar para .CSV", df_historico.to_csv(index=False, sep=';').encode('utf-8'), f"historico_{empresa['nome_fantasia'].replace(' ','_')}_{datetime.now().strftime('%Y%m%d')}.csv", 'text/csv')
            with col2:
                pdf_bytes = self._gerar_pdf(df_historico)
                st.download_button("Exportar para .PDF", pdf_bytes, f"historico_{empresa['nome_fantasia'].replace(' ','_')}_{datetime.now().strftime('%Y%m%d')}.pdf", 'application/pdf')
        else:
